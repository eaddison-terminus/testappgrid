...
#   Assumptions: Python Interpreter is 3.8 in a virtual python environment using PyCharm, pytest, markupsafe, lxml and requests libs installed in venv
        ...
        if pat3.search(page.url):
            ...
            for count, aTime in enumerate(times.xpath("td")):
                ...
                if len(aTime.xpath("./div/b/text()")) > 0 and txt2[count] != 'night':
                    print(f'Place: {place}   time: {aTime.xpath("div/b/text()")[0]}  height: {aTime.xpath("div/span/text()")[0]} ft')
        else:
